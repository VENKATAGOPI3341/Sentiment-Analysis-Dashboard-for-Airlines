...
select = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='1a')  # Changed key to '1a'
...
# if not st.sidebar.checkbox("Close", True, key='1b'):  # Changed key to '1b'
...
each_airline = st.sidebar.selectbox('Visualization type', ['Bar plot', 'Pie chart'], key='2a')
...
if not st.sidebar.checkbox("Close", True, key='2b'):
...
if len(choice) > 0:
    ...
    breakdown_type = st.sidebar.selectbox('Visualization type', ['Pie chart', 'Bar plot'], key='3a')
...
if not st.sidebar.checkbox("Close", True, key='3'):
    ...
    # Tokenize the processed words into a list of individual words
    tokenized_words = processed_words.split()

    # Use Counter to count the frequency of each word
    word_counts = Counter(tokenized_words)

    # Display the top words with their frequencies
    st.subheader(f'Top Words for {word_sentiment} sentiment')
